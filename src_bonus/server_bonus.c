...
/*   Updated: 2024/09/17 17:20:44 by mde-souz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * the binary of a number from the ASCII table
 * that is being received by the server.
 * If the signal is SIGUSR1, the value of `g_byte` is set to '0'.
 * If the signal is SIGUSR2, the value of `g_byte` is set to '1'.
 * 
 * When all the 8 bits are received,
 * the function converts the byte to a character,
 * prints it and sends a SIGUSR1 signal back 
 * to the process that sent the signal using the `kill` function.
 * 
 * @param sig The signal number.
 * @param si Pointer to a siginfo_t structure (information about the signal)
 * @param context A pointer to a ucontext_t structure (signal context)
 */
 ...
 * It initializes signal handlers and enters an infinite loop.
 *
 * Signal Handlers:
